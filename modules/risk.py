...
class RiskGovernor:
    ...
    def get_dynamic_slippage_bps(self, tier, theoretical_bps):
        ...
        if observed_p95:
            ...
            if effective > theoretical_bps:
                print(
                    f"⚠️ RISK GOVERNOR: Slippage Inflation Active for {tier}. Model: {theoretical_bps}bps -> Real: {effective:.0f}bps"
                )
            return effective
    ...
    def check_kill_switch(self, current_vix, dynamic_threshold=None, drawdown_rate_weekly=None):
        ...
        """
        if (
            drawdown_rate_weekly is not None
            and drawdown_rate_weekly > self.drawdown_rate_kill_weekly
        ):
            msg = (
        ...
        if current_vix > limit:
            self.log_rejected_trade(
                "PORTFOLIO", f"Kill Switch Active: VIX {current_vix:.2f} > {limit:.2f}", current_vix
            )
    ...
    def calculate_max_capital_at_risk(
        self, total_capital, current_drawdown_pct, regime_details=None
    ):
        """
    ...
    def validate_correlation_risk(self, portfolio_avg_corr):
        ...
        if portfolio_avg_corr > self.corr_liquidate_threshold:
            ...
            print(f"RISK: Correlation emergency ({portfolio_avg_corr:.2f}) -> Full de-risk")
            return 0.0
            ...
            print(f"RISK: Correlation spike ({portfolio_avg_corr:.2f}) -> Reducing exposure 20%")
            return 0.80
    ...
    def validate_var_budget(self, projected_var_pct, max_var_pct):
        ...
        if projected_var_pct > max_var_pct:
            msg = f"VaR Limit Breach: {projected_var_pct:.2f}% > {max_var_pct:.2f}%"
