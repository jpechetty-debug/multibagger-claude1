...
def _extract_row_value(frame: pd.DataFrame, labels: list[str], col) -> float | None:
...
def calculate_cagr(start: float | None, end: float | None, years: int) -> float:
...
async def process_fiscal_year_data(
    ...
    try:
        ...
        ps = (
            (price / sales_per_share)
            if sales_per_share is not None and sales_per_share > 0
            else None
        )
        pb = (
...
def calculate_divergence_analysis(data_points: list[dict]) -> dict:
    ...
    sales_cagr = calculate_cagr(
        data_points[0].get("sales_per_share"), data_points[-1].get("sales_per_share"), years
    )
    book_value_cagr = calculate_cagr(
        data_points[0].get("book_value"), data_points[-1].get("book_value"), years
    )
