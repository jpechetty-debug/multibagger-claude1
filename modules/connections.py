...
from db.db_core import get_db_connection as get_sqla_connection, db_engine
    ...
    for attempt in range(SQLITE_WRITE_RETRIES):
        try:
            ...
            if _is_sqlite_lock_error(exc) and attempt < SQLITE_WRITE_RETRIES - 1:
                ...
                runtime_logger.warning("SQLite lock during async write; retrying", 
...
def _run_sqlite_write_with_retry_sync(write_fn: Callable[[], Any], operation_name: str):
    for attempt in range(SQLITE_WRITE_RETRIES):
        try:
            ...
            if _is_sqlite_lock_error(exc) and attempt < SQLITE_WRITE_RETRIES - 1:
                wait = SQLITE_RETRY_BASE_SECONDS * (2 ** attempt)
                runtime_logger.warning("SQLite lock during sync write; retrying", 
                                      operation=operation_name, 
                                      wait_seconds=round(wait, 2), 
                                      attempt=attempt + 1)
                time.sleep(wait)
