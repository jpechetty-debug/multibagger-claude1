...
DB_NAME = resolve_db_path(_db_raw)
...
def get_connection():
    ...
    For SQLite: applies WAL mode and busy_timeout pragmas via db_utils.
...
def _write_fundamentals_snapshot(df_db):
    ...
    try:
        ...
        _log.warning(f"Warning: PITDataStore sync failed: {e}")
...
def _detect_score_drift(df_new: "pd.DataFrame"):
    """Compare new scores against existing DB and log drift alerts.

    Triggered before save_multibaggers writes. Any stock whose score
    changes by >SCORE_DRIFT_THRESHOLD without a corresponding fundamental
    change is flagged as a potential regression or data corruption.
    """
    if "symbol" not in df_new.columns or "score" not in df_new.columns:
        return
    ...
    try:
        ...
        try:
            ...
            existing = pd.read_sql(
                f"SELECT symbol, score, {', '.join(_FUNDAMENTAL_FIELDS)} FROM multibaggers",
                conn,
            )
            if existing.empty:
                return

            # Ensure score_drift_alerts table exists
            conn.execute("""
                CREATE TABLE IF NOT EXISTS score_drift_alerts (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    symbol TEXT NOT NULL,
                    old_score REAL,
                    new_score REAL,
                    drift_points REAL,
                    fundamental_changed INTEGER DEFAULT 0,
            ...
            merged = df_new.merge(
                existing, on="symbol", how="inner", suffixes=("_new", "_old")
            )
            if merged.empty:
                return
            ...
            for _, row in merged.iterrows():
                old_score = row.get("score_old")
                new_score = row.get("score_new")
                if old_score is None or new_score is None:
                    continue

                try:
                    drift = abs(float(new_score) - float(old_score))
                except (TypeError, ValueError):
                    continue
                ...
                # Check if any fundamental field changed significantly
                fundamental_changed = 0
                for f in _FUNDAMENTAL_FIELDS:
                    old_val = row.get(f"{f}_old")
                    new_val = row.get(f"{f}_new") if f"{f}_new" in row.index else row.get(f)
                    if old_val is not None and new_val is not None:
                        try:
                            if abs(float(new_val) - float(old_val)) > 0.01:
                                fundamental_changed = 1
                                break
                        except (TypeError, ValueError):
                            continue
            ...
            if drift_records:
                conn.executemany(
                    """INSERT INTO score_drift_alerts
                       (symbol, old_score, new_score, drift_points, fundamental_changed)
                       VALUES (?, ?, ?, ?, ?)""",
                    drift_records,
                )
                conn.commit()
                unexplained = sum(1 for r in drift_records if r[4] == 0)
                _log.info(f"Score drift: {len(drift_records)} alerts "
                    f"({unexplained} unexplained by fundamentals)")
        finally:
            conn.close()
    except Exception as exc:
        _log.warning(f"Warning: Score drift detection failed: {exc}")
...
def init_db():
    ...
    """)
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_score_drift_symbol
        ON score_drift_alerts (symbol, detected_at)
    """)
    ...
    _log.info("Database initialized via db/repository.py.")
...
def save_multibaggers(df, *, replace_existing: bool = False):
    ...
    _log.info(f"DEBUG DB: Saving {len(df)} stocks. Available cols: {available_cols}")
    ...
    try:
        ...
        _log.warning(f"Warning preserving audit logs: {e}")
    ...
    # Phase 4.1: Score drift detection before write
    _detect_score_drift(df_db)
    ...
    try:
        ...
        _log.warning(f"Warning: PIT retention prune skipped: {exc}")
    _log.info("Saved Multibaggers to DB (Schema Preserved).")
...
def save_microcaps(df):
    ...
    conn.close()
    _log.info("Saved Microcaps to DB.")
