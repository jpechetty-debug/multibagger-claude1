...
class YFinanceSource(DataSource):
    ...
    def fetch_quarterly_results(self, symbol: str) -> list[dict]:
        try:
            ...
            revenue_key = next(
                (k for k in q_financials.index if "Total Revenue" in k or "Operating Revenue" in k),
                None,
            )
            profit_key = next(
                (k for k in q_financials.index if "Net Income" in k or "Net Profit" in k), None
            )
            ...
            profits = (
                q_financials.loc[profit_key].fillna(0)
                if profit_key
                else pd.Series([0] * len(revenues), index=revenues.index)
            )
            ...
            )  # Sort logic might need date parsing check
