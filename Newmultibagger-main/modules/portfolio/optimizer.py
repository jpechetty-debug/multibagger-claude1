...
class PortfolioOptimizer:
    ...
    def optimize_allocation(self, stocks, history_df=None, method="risk_parity"):
        ...
        if method == "hrp" and history_df is not None and not history_df.empty:
            _log.info("[INFO] Applying HRP Allocation...")
            ...
            df["Volatility"] = df.apply(
                lambda x: (x.get("ATR", 0) / x.get("Price", 1)) if x.get("Price", 1) > 0 else 0.05,
                axis=1,
            )
            ...
            penalty_map = dict.fromkeys(df["Symbol"], 1.0)

            # --- CORRELATION PENALTY (v2.4) ---
            if history_df is not None and not history_df.empty:
                _log.info("[INFO] Applying Correlation Penalty...")
                ...
                for i in range(len(symbols)):
                    for j in range(i + 1, len(symbols)):
                        ...
                        if correlation > 0.7:
                            ...
                            _log.info(f"  > High Corr ({correlation:.2f}) {s1} vs {s2}. Penalty Factor: {penalty_factor:.2f} on {victim}.")
        ...
        for sec, total_w in sec_map.items():
            if total_w > self.max_sector_weight:
                ...
                try:
                    ...
                    rg = RiskGovernor()
                    rg.log_rejected_trade(
                        f"SECTOR:{sec}",
                        f"Sector Capped {total_w:.1%} > {self.max_sector_weight:.1%}",
                        0.0,
                    )
                except Exception as e:
                    _log.error(f"[ERROR] Logging Failed: {e}")
        ...
        _log.info(f"Optimization Result: Invested {total_allocated_weight:.1%}, Cash {cash_weight:.1%}")
