...
def run_slippage_analysis():
    ...
    if "market_cap_cr" in df.columns:
        portfolio["market_cap_cr"] = pd.to_numeric(
            portfolio["market_cap_cr"], errors="coerce"
        ).fillna(0)
    else:
    ...
    portfolio["net_return"] = (
        portfolio["gross_return"] - (portfolio["slippage_pct"] * 2) - txn_costs
    )

    # 5. Analysis
    avg_gross = portfolio["gross_return"].mean()
    avg_net = portfolio["net_return"].mean()
    portfolio["slippage_pct"].mean()
    ...
    if not microcaps.empty:
        print(
            microcaps[["symbol", "market_cap_cr", "gross_return", "net_return"]].to_string(
                index=False
            )
        )
    else:
    ...
    with open("slippage_report.md", "w", encoding="utf-8") as f:
        ...
        f.write(
            portfolio[
                ["symbol", "market_cap_cr", "slippage_pct", "gross_return", "net_return"]
