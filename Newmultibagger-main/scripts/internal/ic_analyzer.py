...
def fetch_current_prices(symbols):
    """Fetch current prices from the multibaggers table (instant)."""
    try:
        conn = sqlite3.connect(DB_PATH)
        query = f"SELECT symbol, price FROM multibaggers"
        df = pd.read_sql(query, conn)
        conn.close()
        return dict(zip(df["symbol"], df["price"]))
    except Exception as e:
        print(f"Failed to fetch current prices from multibaggers: {e}")
        return {}
...
def perform_ic_analysis():
    ...
    print(f"Fetching current prices for {len(symbols)} symbols to calculate forward returns...")
    ...
    if df.empty:
        print("No valid forward returns calculable (missing current prices).")
        return
