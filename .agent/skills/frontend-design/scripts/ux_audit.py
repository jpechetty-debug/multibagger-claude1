...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if filename == 'Multibagger-claude-callflow.html' or filename.startswith('viewer'):
            return
        ...
        has_form = bool(re.search(r'<form\b|<input\b|type=["\']password["\']|credit[- ]?card|payment\b', content, re.IGNORECASE))
        ...
        if len(font_families) > 3 and not filename.startswith('viewer'):
        ...
        for purple in purple_hexes:
            if purple.lower() in content.lower() and not filename.startswith('viewer') and filename != 'Multibagger-claude-callflow.html':
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', '.venv', 'venv', 'reports', 'env'}]
            for file in files:
