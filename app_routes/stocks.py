    ...
    try:
        ...
        def _normalize_valuation_payload(payload: dict):
            ...
            def _component_or_none(value):
                ...
                if not np.isfinite(parsed) or parsed <= 0: return None
                return parsed
        ...
        info = await run_with_exponential_backoff(lambda: deps._run_blocking(lambda: ticker.info), context=f"yf valuation {symbol}")
        if not info: return {"error": f"Failed to fetch valuation for {symbol}"}
        ...
        data = {
            ...
            "free_cash_flow_per_share": ((info.get("operatingCashflow", 0) - abs(info.get("capitalExpenditures", 0))) / info.get("sharesOutstanding", 1)) if info.get("operatingCashflow") else 0,
...
async def get_governance_data(symbol: str):
    ...
    try:
        ...
        def _fetch_gov_data():
            ticker = yf.Ticker(symbol)
            info = ticker.info
            def get_val(key, default=None): return info.get(key, default) or default
            sector = get_val('sector', 'Unknown')
            roe = round(get_val('returnOnEquity', 0) * 100, 2)
            de = round(get_val('debtToEquity', 0) / 100, 2)
            ...
            ni = get_val('netIncomeToCommon', 1)
            return {
                "symbol": symbol, "sector": sector, "is_financial": 'Financial' in sector or 'Bank' in sector,
                "roe": roe, "debt_to_equity": de, "sales_growth": sales_growth, "profit_growth": profit_growth,
                "promoter_holding": promoter, "pledged_pct": 0, "cfo_pat_ratio": round(cfo / ni, 2) if ni else 0
            }
