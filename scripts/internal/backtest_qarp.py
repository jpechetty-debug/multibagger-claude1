...
def get_pit_factors(symbol, target_date, cache=None):
    try:
        ...
        available_quarters = [
            d for d in dates if (d + reporting_lag) <= pd.to_datetime(target_date)
        ]
...
def calculate_risk_metrics(rets, bench_rets=None):
    ...
    if bench_rets is not None:
        ...
        alpha = ann_ret - (
            (1 + (np.prod(1 + np.array(bench_rets) / 100) - 1)) ** (4 / len(rets)) - 1
        )
...
def run_backtest(years=3, universe_size=50, stress_mode=False, rebal_freq="MS"):
    ...
    if df_results.empty:
        return
    m = calculate_risk_metrics(
        df_results["period_ret"].tolist(), df_results["benchmark_ret"].tolist()
    )
    ...
    with open("qarp_backtest_report.md", "w") as f:
        ...
        f.write("- Regime Detection: Gaussian HMM (Bullish/Volatile/Bearish)\n")
        ...
        f.write("- Slippage Modeling: Tiered (0.2% - 2.0%)\n")
        f.write("- Transaction Costs: 0.2% per round-trip\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rebal", type=str, default="MS", help="Rebalance frequency (e.g., MS, 3MS, W)"
    )
