...
from sovereign_cli import _run_paper_trade_scan, cmd_paper_trade, cmd_db_dups
...
async def test_cmd_paper_trade_filters_score(mock_signals_log, monkeypatch):
    ...
    monkeypatch.setattr(sovereign_cli, "_run_paper_trade_scan", lambda *a: [
    ...
    with open(mock_signals_log, "r") as f:
        history = json.load(f)
...
async def test_db_dups_forensic():
    ...
    with patch("sovereign_cli.sqlite3.connect") as mock_connect, \
