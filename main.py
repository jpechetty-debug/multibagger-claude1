...
from worker.background_jobs import (
...
async def update_prices_background():
    """Compatibility wrapper around the standalone background price worker."""
...
@app.get("/api/regime_status", response_model=RegimeStatusResponse)
async def get_regime_status():
    ...
    try:
        ...
        if (
            _cache_is_fresh(regime_cache, REGIME_CACHE_TTL_SECONDS)
            and regime_cache.get("key") == cache_key
        ):
...
def force_regime(regime: str):
    ...
    try:
        ...
        if regime == 'AUTO':
            ...
            runtime_logger.info("Regime forced by administrator", regime=regime)
...
async def get_valuation(symbol: str, as_of_date: str | None = None):
    try:
        ...
        api_logger.error("Failed to load valuation metrics", symbol=symbol, error=str(e))
        return {"error": str(e)}
...
async def quarterly_results_endpoint(symbol: str, quarters: int = 12):
    ...
    try:
        ...
        if _cache_is_fresh(CACHE_QUARTERLY.get(symbol, {}), CACHE_AUDIT_TTL):
            api_logger.info("Quarterly results cache hit", symbol=symbol)
            return CACHE_QUARTERLY[symbol]["payload"]

        api_logger.info("Quarterly results request started", symbol=symbol)
...
async def price_fundamentals_endpoint(symbol: str, years: int = 5):
    ...
    try:
        ...
        # Check Cache
        if _cache_is_fresh(CACHE_FUNDAMENTALS.get(cache_key, {}), CACHE_AUDIT_TTL):
            api_logger.info(
                "Price fundamentals cache hit",
                symbol=symbol,
                years=years,
            )
            return CACHE_FUNDAMENTALS[cache_key]["payload"]
        ...
        # Set Cache
        if cache_key not in CACHE_FUNDAMENTALS: CACHE_FUNDAMENTALS[cache_key] = {}
        _cache_set(CACHE_FUNDAMENTALS[cache_key], cleaned)
