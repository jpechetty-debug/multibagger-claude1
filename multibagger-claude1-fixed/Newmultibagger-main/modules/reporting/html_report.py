...
async def generate_premium_html_report(symbol: str):
    ...
    if os.path.exists(output_path):
        ...
        if (datetime.now().timestamp() - mtime) < 300:  # Reduced to 5 minutes
            _log.info(f"Returning cached Premium Report for {symbol}...")
            return output_path

    _log.info(f"Generating Premium Audit Report for {symbol}...")
    ...
    is_finance = any(
        x in (sector + industry).lower() for x in ["bank", "finance", "financial", "nbfc"]
    )
    ...
    # C1: ROE
    roe = (
        _safe_float(f_override.get("ROE%")) / 100
    ...
    # C2: Debt/Equity
    de = (
        _safe_float(f_override.get("Debt_Equity"))
        if "Debt_Equity" in f_override
        else _safe_float(info.get("debtToEquity"))
    )
    if is_finance:
        c2_pass = True
        de_str = "N/A (Bank)"
    ...
    # C3: Sales Growth
    rev_growth = (
        _safe_float(f_override.get("Sales_Growth_TTM%")) / 100
    ...
    # C4: Profit Growth
    profit_growth = (
        _safe_float(f_override.get("EPS_Growth%")) / 100
        if "EPS_Growth%" in f_override
        else _safe_float(info.get("earningsGrowth"))
    )
    ...
    rec = (
        ...
        if manual_rec
        else str(info.get("recommendationKey", "HOLD")).replace("_", " ").upper()
    )
    target = (
        _safe_float(manual_target) if manual_target else _safe_float(info.get("targetMeanPrice"))
    )
    ...
    context["valuation"]["verdict"] = (
        "STRONG BUY" if rec in ["BUY", "STRONG BUY"] and mos > 20 else "NEUTRAL"
    )
    if mos < 0:
        context["valuation"]["verdict"] = "OVERVALUED"
    ...
    context["governance"] = {
        ...
        "has_red_flags": any(
            info.get(r, 0) and info.get(r, 0) > 7
            for r in ["overallRisk", "auditRisk", "boardRisk", "shareHolderRightsRisk"]
        )
        or short_int > 5,
    }
    ...
    # --- 8. Targets ---
    context["targets"] = {"conservative": f"{price * 1.10:,.2f}", "bull": f"{price * 1.30:,.2f}"}

    context["entry_zone"] = {"low": f"{price * 0.95:,.2f}", "high": f"{price:,.2f}"}

    roe_pct = round(roe * 100, 1) if (roe and roe > 0) else 0.0
    consistency = q_data.get("trends", {}).get("consistency", "UNKNOWN")
    context["thesis"] = {
        "title": "Champion in Credit Execution"
        if is_finance
        else "Market Leadership in Core Segment",
        "points": [
            f"Dominant footprint within the {industry} space.",
            f"{roe_pct}% Return on Equity, demonstrating strict capital efficiency.",
            "Consistent quarterly execution metrics."
            if consistency == "HIGH"
            else "Volatile but recovering quarterly execution.",
        ],
    }
    ...
    _log.info(f"Report generated: {output_path}")
    return output_path
